...
use Api\General\Models\Position;
...
trait LaratrustUserTrait
{
    ...
    /**
     * Many-to-Many relations with position.
     *
     * @return \Illuminate\Database\Eloquent\Relations\MorphToMany
     */
    public function userPositions()
    {
        return $this->hasMany(Config::get('laratrust.models.position'), Config::get('laratrust.foreign_keys.user'));
...
//        return $roles;
     ...
     * @return \Illuminate\Database\Eloquent\Collection
     */
    public function cachedPositionPermissions()
    {
        $cacheKey = 'laratrust_position_permissions_for_user_' . $this->getKey();
    ...
    public function positionPermissions()
    {
        ...
        foreach ($this->positions()->get() as $position) {
            $positions->push($position->permissions()->get());
        }

        return $positions->flatten()->pluck('name')->toArray();
    }
    ...
    public function hasPermission($permission, $team = null, $requireAll = false)
    {
        ...
        foreach ($this->positionPermissions() as $permiss) {
            if (Helper::isInSameTeam($permiss, $team) && str_is($permission, $permiss)) {
                return true;
            }
        }

        foreach ($this->userPositions() as $position) {
            if (Helper::isInSameTeam($perm, $team) && str_is($permission, $position['name'])) {
                return true;
            }
        }
    ...
    {
        $cacheKey = 'laratrust_positions_for_user_' . $this->getKey();

        if ( ! Config::get('laratrust.use_cache')) {
            return $this->userPositions()->get();
        }

        return Cache::remember($cacheKey, Config::get('cache.ttl', 60), function () {
            return $this->userPositions()->get()->toArray();
        });
    }
