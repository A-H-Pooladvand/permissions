...
class LaratrustMiddleware
{
     ...
     * @return boolean
     */
    protected function authorization($type, $permissions, $team, $options)
    {
        ...
        return ! Auth::guard($guard)->guest() && Auth::guard($guard)->user()->$method($permissions, $team, $requireAll);
    }

    protected function getMethodName(string $type)
    {
        switch ($type) {
            case 'roles':
                return 'hasRole';
